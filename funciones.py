...
def digitacion(cur,codigo):
    resultado=[]
    cur.execute(f"SELECT username,password FROM digitadores WHERE codigo='{codigo}'")
    result = cur.fetchall()
    for valor in result:
      resultado = valor[0]
      resultado1 = valor[1]
...
def listado_digitadores(cur):
     digitador=[]
     cur.execute("SELECT username FROM digitadores") 
     digitacion=cur.fetchall()
     for digitadores in digitacion:
...
def listado_paradas(cur):
    ...
    paradax = cur.fetchall() 
    for valor in paradax:
        parada += valor 
    return parada
...
def estado_bancario(cur,parada,fecha,banco,numero_cuenta,balance): 
    cur.execute(f"CREATE TABLE IF NOT EXISTS {parada}_banco( fecha VARCHAR(50)  NULL, banco VARCHAR(50) NULL,  numero_cuenta VARCHAR(50) NULL, balance DECIMAl(10,2) unsigned DEFAULT 0)")                                                                                                                                
    cur.execute(f"INSERT INTO {parada}_banco(fecha, banco, numero_cuenta, balance) VALUES('{fecha}', '{banco}',{numero_cuenta} {balance})")
...
def actualizar_asoc(cur,parada,nombre,cedula,telefono,funcion,identificador):
     cur.execute(F"UPDATE {parada} SET nombre='{nombre}',cedula='{cedula}',telefono='{telefono}',funcion='{funcion}' WHERE nombre='{identificador}' ") 
     return 
     
def listado_miembro(cur) :
    nombre=[]
    cur.execute("SELECT funcion FROM administracion")
    funcion=cur.fetchall()
    for valor in funcion:
        nombre +=valor
    return nombre
